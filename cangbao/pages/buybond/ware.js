...
Page({
  ...
  onShow: function () {
    ...
    setTimeout(function () {
      ...
      if (Number(that.data.details.sell_price) <= Number(balance)) {
        that.setData({
          paytype: 'balance',
          isrecharge: 0
        })
  ...
  godetails(){
    const that=this
    wx.redirectTo({
      url: '../orderdetails/orderdetails?id=' + that.data.order_id,
    })
  },
  pays(){
    var that=this
          ...
          success: function (res) {
            if (that.data.paytype == "balance") {
              if (res.data.status_code !== 200) {
                ...
                wx.redirectTo({
                  url: '../paysuccess/paysuccess?id=' + that.data.order_id
                })
              }
            } else {
  ...
  ispay(){
    ...
    if (that.data.paytype == 'balance') {
      ...
      if (that.data.details.sell_price > 50000) {
        that.setData({
          quota: 1
        })
      } else {
  ...
  gopares(){
    let that=this
    wx.navigateTo({
      url: '../person/person?id=' + that.data.details.user.user_id,
    })
  },
  // 跳转支付限额说明
  goquota() {
